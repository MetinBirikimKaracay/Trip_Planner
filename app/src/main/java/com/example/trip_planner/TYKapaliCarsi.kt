...
import kotlinx.android.synthetic.main.activity_tykapali_carsi.*

class TYKapaliCarsi : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_tykapali_carsi)
        ...
        if(totalPoint == 0)
        {
            ratingBarTYKapaliCarsi.rating = 3F
        }
        else if(totalPoint > 0)
        {
            var rate = totalPoint / counter
            //ratingbarın idsini düzelt
            ratingBarTYKapaliCarsi.rating = rate.toFloat()
        }
